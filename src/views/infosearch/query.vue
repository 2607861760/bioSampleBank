      ...
      <div class="process-inner" v-if="filtrateState">
        <el-form ref="form" :model="basicform" label-width="100px" label-position="left">
          ...
          <el-form-item label="癌       种:">
            <el-checkbox-group v-model="ctype"> 
          ...
          <el-form-item label="标本类型:">
            <el-checkbox-group v-model="btype">
              <el-checkbox v-for="(item,index) in specimentypeList" :label="index" :key="index">{{item}}</el-checkbox>
            </el-checkbox-group>
          </el-form-item>
          <el-divider></el-divider>
          <div>
            <el-form-item label="原发灶情况：">
              <el-radio-group v-model="basicform.prinature" >
                <el-radio v-for="(item,index) in primaryList" :label="index" :key="index">{{item}}</el-radio>
              </el-radio-group>
            </el-form-item>
            <el-divider></el-divider>
            <el-form-item label="肿瘤部位：">
              <el-checkbox-group v-model="tumloc" :max='basicform.prinature==0?1:tumourdepartList.length-1'>
                <el-checkbox v-for="(item,index) in tumourdepartList" :label="index" :key="index">{{item}}</el-checkbox>
              </el-checkbox-group>
            </el-form-item>
            <el-divider></el-divider>
            <el-form-item label="临床分期：">
              <el-checkbox-group v-model="clinicalStage">
                <el-checkbox v-for="(item,index) in clinicalstagesList" :label="index" :key="index">{{item}}</el-checkbox>
              </el-checkbox-group>
            </el-form-item>
            <el-divider></el-divider>
          </div>
          <div>
            <el-form-item label="检测标本：">
              <el-checkbox-group v-model="testSpecimen">
                <el-checkbox v-for="(item,index) in specimenList" :label="index" :key="index">{{item}}</el-checkbox>
              </el-checkbox-group>
            </el-form-item>
            <el-divider></el-divider>
            <el-form-item label="肿瘤分期：">
              <el-checkbox-group v-model="tumorStage">
                <el-checkbox v-for="(item,index) in tumourstagesList" :label="index" :key="index">{{item}}</el-checkbox>
              </el-checkbox-group>
            </el-form-item>
            <el-divider></el-divider>
          </div>
...
<script>
import {infosearch} from 'api/index.js';
export default {
  data() {
    return {
      samdate:{},
      ...
      primaryList: ["单发性", "多发性"],
      tumourdepartList: ["末端回肠", "盲肠","阑尾","回盲瓣","肝曲","横结肠","脾曲","降结肠（左半结肠）","乙状结肠","直肠乙状结肠","直肠","升结肠（右半结肠）"],
  ...
  methods: {
    ...
    ctypechange(){
      console.log(this.basicform.prinature)
    },
    stringTonum(list){
      let newlist=[]
      list.map(item=>{
    ...
    query(){
      ...
      this.basicform['age']=this.age;
      this.basicform['weight']=this.weight;
      this.basicform['bmi']=this.bmi;
      this.basicform['systolic']=this.systolic;
      this.basicform['diastolic']=this.diastolic;
      this.basicform['ctype']=this.ctype;
      this.basicform['btype']=this.btype;
